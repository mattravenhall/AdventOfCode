...
def play_bingo(balls, boards):
	for ball in balls:
		update_boards = False
		for board in boards:
			# Update boards, catch if one wins
			if board.update_board(ball):
				update_boards = True
				# If this is the last board winning, get the score
				if len(boards) == 1:
					board_total = board.total()
					score = int(board_total * ball)
					print(score)
					return
		# If a board won, remove it
		if update_boards:
			boards = [board for board in boards if not board.winning]
			update_boards = False
	else:
		return "No win"
